...
async def index_file(
    request: Request, file: UploadFile = File(...), update: bool = Form(False)
):
    """
    ...
    try:
        ...
        obs_name = f"{COLLECTION_NAME}/{file_name}"
...
async def delete_file(request: Request, file_name: str):
    ...
    try:
        jreq = JobRequest(
            function="delete_file", collection=COLLECTION_NAME, file_path=file_name
        )
        job_id = await RedisClient.enqueue_job(jreq)
        return {"job_id": job_id}
