...
async def index_file(
    request: Request,
    file: UploadFile = File(..., description="File to be indexed"),
    update: bool = Form(False, description="Update existing file if true"),
    ...
    try:
        ...
        if MinIOClient.object_exists(obs_name):
            if not update:
                raise HTTPException(
                    status_code=400,
                    detail="File already exists, use update=True if you want to update the file",
                )
            logger.debug(f"File '{file_name}' already exists, updating...")

        # Upload to MinIO
        success = MinIOClient.put_object(
            object_name=obs_name, data=content, content_type=content_type
        )
...
@router.delete("/{collection}/{file_name}")
@limiter.limit(f"{RATE_LIMIT_DELETE}/minute")
async def delete_file(request: Request, collection: str, file_name: str):
    """
